...
public class Pizarra extends javax.swing.JFrame {
    ...
    public Pizarra() {
        ...
        numeroFigura = new JLabel("No.Figura: ");
        ...
        try {
            ...
            for (int i = 0; i < aFiguras.size(); i++) {
                ...
                if (f instanceof Linea) {
                    Linea l = (Linea) f;
                    linea = String.format("L,%.0f,%.0f,%.0f,%.0f,%x\n", l.puntoA.getX(), l.puntoA.getY(),
                            l.puntoB.getX(), l.puntoB.getY(),
                            l.color.getRGB());
                }
                ...
                if (f instanceof Circulo) {
                    ...
                    linea = String.format("Cr,%.0f,%.0f,%.0f,%.0f,%x\n", cr.puntoC.getX(), cr.puntoC.getY(),
                            cr.puntoR.getX(), cr.puntoR.getY(), cr.color.getRGB());
                }

                if (f instanceof Cuadrado) {
                    Cuadrado cd = (Cuadrado) f;
                    linea = String.format("Cd,%d,%d,%d,%d,%x\n", cd.puntoA.getX(), cd.puntoA.getY(),
                            cd.puntoB.getX(), cd.puntoB.getY(), cd.color.getRGB());
                }
                if (f instanceof Elipse) {
                    Elipse el = (Elipse) f;
                    linea = String.format("E,%.0f,%.0f,%.0f,%.0f,%.0f,PA,%.0f,%x\n", el.puntoC.getX(), el.puntoC.getY(),
                            el.puntoR.getX(), el.puntoR.getY(), el.puntoA.getX(), el.puntoA.getY(), el.color.getRGB()
                    );
    ...
    public void dibujarLinea(Point _p1, Point _p2, Color color, boolean b) {
        ...
        if (b) {
            Linea l = new Linea(_p1, _p2, color);
            aFiguras.add(l);
        }
    }
    ...
    public void dibujarCuadrado(Point p1, Point p2, Color c, Boolean b) {
        ...
        if (b) {
            Cuadrado cd = new Cuadrado(p1, p2,v1,v2, color);
            aFiguras.add(cd);
        }
    }
    ...
    public void dibujarCirculo(Point pc, Point pr, Color color, boolean b) {
        ...
        if (b) {
            Circulo cr = new Circulo(pc, pr, color);
            aFiguras.add(cr);
        }
        double x2, y2;
        double angulo = 0;
    ...
    public void dibujarElipse(Point pc, Point pr, Point pa, Color color, boolean t) {
        if (t) {
            Elipse el = new Elipse(pc, pr, pa, color);
            aFiguras.add(el);
        }
        Point pr1 = new Point(pr.x,pr.y);
        Point pc1 = new Point(pc.x,pc.y);
